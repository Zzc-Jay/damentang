  ...
  render() {
    ...
    for (let d = 0; d < this.digits; d++) {
      ...
      upBtn.addEventListener('click', () => {
        this.current[d] = (this.current[d] + 1) % 10;
        this.render();
      });
      ...
      downBtn.addEventListener('click', () => {
        this.current[d] = (this.current[d] + 9) % 10;
        this.render();
      });
  ...
  render() {
    ...
    this.tiles.forEach((tile, idx) => {
      ...
      cell.addEventListener('click', () => {
        ...
        if (this.matched.has(tile.pairIdx)) return;
        if (this.flipped.find(f => f.idx === idx)) return;
        if (this.flipped.length >= 2) return;

        this.flipped.push({ idx, tile });
        if (this.audio) this.audio.sfxClick();
        this.render();
        ...
        if (this.flipped.length === 2) {
          ...
          if (a.tile.pairIdx === b.tile.pairIdx) {
            this.matched.add(a.tile.pairIdx);
            this.flipped = [];
            this._locked = false;
            if (this.audio) this.audio.sfxCollect();
            this.render();
...
class SymbolInputPuzzle {
  ...
  render() {
    ...
    if (this.input.length === this.answer.length) {
      ...
      submitBtn.textContent = '确认';
      submitBtn.addEventListener('click', () => {
        const match = this.input.every((v, i) => v === this.answer[i]);
        if (match) {
          this.onSolve();
        } else {
          this._error = true;
